...
%%
pause(0.3);
[hwave,Cbox]=wave_init();
stop_flag


x_extend=ones(1,zlimit_room)*(-zrelative)/tand(relativeangle); % zrelate ~= -25
...
length(step_length_z)

while stop_flag~=1 && (step_v*step_count)<=length(step_length_z);
xposition3=xposition2-step_length_x(step_v*step_count); %xposition2>xposition1 %going to xposition3=xposition1;
yposition3=yposition1;
zposition3=zposition1+step_length_z(step_v*step_count);%zrelative is negative, so default_zrefer>zposition1 %going to default_zrefer
